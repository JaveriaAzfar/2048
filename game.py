#Project2048
#submittedBy: Javeria Azfar, 
            #  Maleeha Hussain
            #  Rania Siddiqui 
#submittedTo: Sir Abdullah Zafar
#DSA_section: L4
...
class game(tk.Frame):
    def __init__(self):
        ...
        self.main_grid = tk.Frame(self, bg= c.Color_grid, bd = 3 , width=600, height = 400)
    ...
    def make_GUI(self):
        ...
        for i in range(4):
            row = []
            for j in range(4):
                cell_frame = tk.Frame(self.main_grid, bg = c.Color_EmptyCell, width= 150,height=150)
        ...
        tk.Label(score_frame, text = "Score", font= c.Font_ScoreLabel).grid(row = 0)
...
# game()
    def start_game(self):
        ...
        self.matrix[row][col] = 2
        self.cells[row][col]["frame"].configure(bg= c.Color_Cells[2])
        self.cells[row][col]["number"].configure(bg = c.Color_Cells[2],fg=c.Color_CellNumber[2],font = c.Fonts_CellNumebr[2],text = "2")
        self.score = 0
...
#Game OVERRR
    ...
    def game_over(self):
        if any(2048 in row for row in self.matrix):
            ...
            tk.Label(game_over_frame,text = "YOU WIN",bg = c.Winner_BG,fg = c.Font_Color_GameOver,font = c.Font_GameOver).pack()


        elif not any(0 in row for row in self.matrix) and not self.horizontal_move_exists() and not self.verticle_move_exists():
            game_over_frame = tk.Frame(self.main_grid, borderwigth = 2)
            game_over_frame.place (relx = 0.5, rely = 0.5, anchor= "center")
            tk.Label(game_over_frame,text = "Game Over TT",bg = c.Loser_BG,fg = c.Font_Color_GameOver,font = c.Font_GameOver).pack()
...
#code refernced from Kite on Youtube.
# https://www.youtube.com/watch?v=Oh1AYUmvFoo        
